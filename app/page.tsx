...
export default function Home() {
  ...
  if (detectedType) {
    lines.push(`- **Detected project type:** ${typeLabel[detectedType] ?? detectedType}`);
  }
  lines.push(
  ...
  for (const category of order) {
    ...
    sorted.forEach((issue, idx) => {
      lines.push(`#### ${idx + 1}. [${issue.severity.toUpperCase()}] ${issue.title}`);
      lines.push('');
  ...
  // ---- Passed checks (positive context) ----------------------------------
  if (result.passed.length > 0) {
    ...
    for (const [category, checks] of Object.entries(byCategory)) {
      ...
      lines.push(`- **${categoryTitles[category] ?? category}**: ${checks.join(', ')}`);
    }
    lines.push('');
  }
  ...
  lines.push('');
  lines.push(
    `Start with the CRITICAL issues. For each one, show me the file, the diff, and a one-line explanation of what changed. When you finish all criticals, stop and wait for me to review before moving to highs.`
  );
  lines.push('');
  lines.push(
